...
use cuda_rust_wasm::{
    get_neural_capabilities, init_neural_integration, BridgeConfig, NeuralActivationFunction,
    NeuralBridge, NeuralOperation,
};
...
fn demo_basic_neural_bridge() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let matrix_b: Vec<f32> = (0..size * size)
        .map(|i| ((i * 2) as f32) / 1000.0)
        .collect();
    ...
    println!(
        "  Matrix multiplication completed in {:.2}ms",
        duration.as_millis()
    );
    ...
    println!(
        ...
        perf_stats.average_execution_time * 1000.0
    );
    println!(
        "  GPU Utilization: {:.1}%",
...
fn demo_ruv_fann_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let operation = NeuralOperation::ForwardPropagation { layer_sizes };
    ...
    let start = std::time::Instant::now();
    let _cpu_batch_results: Vec<Vec<f32>> = batch_inputs
        .iter()
...
fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in &sizes {
        ...
        println!(
            "  Size {:6}: GPU {:.3}ms, CPU {:.3}ms, Speedup {:.2}x, Throughput {:.1} Mops/s",
            size,
            gpu_time.as_millis(),
            cpu_time.as_millis(),
            speedup,
    ...
    println!(
        "  GPU Allocated: {:.1} MB",
        memory_stats.gpu_allocated as f64 / 1024.0 / 1024.0
    );
    println!(
        "  CPU Allocated: {:.1} MB",
        memory_stats.cpu_allocated as f64 / 1024.0 / 1024.0
    );
    println!(
        "  Peak Usage: {:.1} MB",
        memory_stats.peak_usage as f64 / 1024.0 / 1024.0
    );
...
fn demo_batch_neural_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (name, _)) in network_configs.iter().enumerate() {
        if i < results.len() {
            println!(
                "  {} result: {} outputs, first 3: {:?}",
                name,
                results[i].len(),
                &results[i][0..3.min(results[i].len())]
            );
        }
    }
    ...
    for batch_size in batch_sizes {
        ...
        println!(
            "  Batch size {:2}: {:.2}ms ({:.1} networks/sec)",
            batch_size,
            time.as_millis(),
            throughput
        );
