...
fn main() {
    ...
    // Test 5: Matrix representation operations
    println!("🔷 Test 5: Verifying Matrix Representations");
    println!("-------------------------------------------");
    
    let gl2 = ReductiveGroup::gl_n(2);
    let id1 = MatrixRepresentation::identity(gl2.clone(), 2);
    let id2 = MatrixRepresentation::identity(gl2.clone(), 2);
    
    // Identity composed with identity should be identity
    let composed = id1.compose(&id2).expect("Composition should work");
    let is_identity = composed.matrix.iter().enumerate().all(|(i, &val)| {
        let row = i / 2;
        let col = i % 2;
        if row == col {
    ...
    println!("Identity ∘ Identity = Identity: {}", is_identity);
    assert!(is_identity, "Identity composition failed");
    ...
    println!("  ✓ Matrix representation composition");
}